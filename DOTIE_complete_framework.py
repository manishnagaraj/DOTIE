...
# -*- coding: utf-8 -*-
"""
Code for DOTIE spiking architecture only
"""
...
if __name__=="__main__":
    ...
    for curr_pos in range(evnts_enc.shape[2]):
        ...
        # Perform spatial clustering using DBSCAN and get bounding boxes
        DOTIE_boxes = get_boundaries_DOTIE(recovered_inputs_3chnl, eps_val=15, min_samples_val=10, mindiagonalsquared=2300)
        DOTIE_result = evnt_frame_3chnl
        if DOTIE_boxes != None:
            for bbox in DOTIE_boxes:
                cv2.rectangle(DOTIE_result, (bbox[0], bbox[1]), (bbox[2], bbox[3]), [0,0,0], 3, cv2.LINE_AA)
        
        # Output visual
        visual =  np.concatenate((gryimg_3chnl,  DOTIE_result), axis=1)
        cv2.imshow('DOTIE Framework', cv2.cvtColor(visual, cv2.COLOR_BGR2RGB))
        cv2.waitKey(1)
    print('\n******************Done****************************')